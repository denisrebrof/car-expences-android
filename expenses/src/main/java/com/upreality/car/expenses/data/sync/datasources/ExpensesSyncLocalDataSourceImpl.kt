...
import com.upreality.car.expenses.data.sync.room.expensesinfo.model.queries.*
...
class ExpensesSyncLocalDataSourceImpl @Inject constructor(
    ...
    private fun getLocalExpense(info: ExpenseInfo): Maybe<ExpenseRoom> {
        val filters = ExpenseFilter.Id(info.localId).let(::listOf)
        val dbFilter = RoomExpenseFilterConverter.convert(filters)
